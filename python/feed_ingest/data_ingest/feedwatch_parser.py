...
def append_cost_row(farm_id, row, write_df, record_date):
    ...
    new_row = [int(farm_id), row[1][0].encode('ascii'), int(row[1][1].encode('ascii')), get_dollar_value(row[1][2]), get_dollar_value(row[1][3]), record_date]
    ...
    with db_engine.connect() as con:
        for row in cost_df.itertuples():
            ...
            data = ({"farm_id":row.farm_id, "pen_name":row.pen_name, "report_date":row.report_date, \
                "avg_pen_count":row.avg_pen_count, "expected_cost":row.expected_cost, "actual_cost":row.actual_cost})
...
def import_dmi_from_file(input_df, farm_id):
    ...
    for row in input_df.iterrows():
        ...
        # normal row parsing operation
            ...
            if type(first_col) in (str, unicode):
                if first_col:
...
def append_dmi_row(farm_id, company_name, pen_name, row, write_df):
    ...
    try:
        ...
        get_a_float(row[1][4]), get_a_float(row[1][5]), get_a_float(row[1][6]), get_a_float(row[1][7]), get_a_float(row[1][8])]
...
def write_dmi_to_temp(dmi_df, db_engine, uuid_str):
    ...
    insert_statement = text('INSERT INTO ' + table_name + ' (farm_id, company_name, pen_name, report_date, avg_pen_count, '\
    ...
    with db_engine.connect() as con:
        for row in dmi_df.itertuples():
            ...
            data = ({"farm_id":row.farm_id, "company_name":row.company_name, "pen_name":row.pen_name, "report_date":row.report_date, \
                "avg_pen_count":row.avg_pen_count, "total_dropped":row.total_dropped, "avg_dropped":row.avg_dropped, \
                "total_weighbacks":row.total_weighbacks, "avg_weighbacks":row.avg_weighbacks, "total_dm":row.total_dm, \
...
def import_ingredients_from_file(input_df, farm_id):
    # create write_df
    db_column_names = ["farm_id", "company_name", "pen_type", "ingredient_name", "as_fed_quantity", "as_fed_tons", "report_date"]
    write_df = pd.DataFrame(columns=db_column_names)
    last_was_totals = False
...
def append_ingredient_row(farm_id, company_name, pen_type, row, write_df, record_date):
    ...
    new_row = [int(farm_id), company_name, pen_type, row[1][0], get_a_float(row[1][2]), get_a_float(row[1][3]), record_date]
    logging.debug('appending new ingredient row: ' + str(new_row))
    # append the data to the dataframe in the last position
    write_df.loc[len(write_df)] = new_row 
...
def write_ingredients_to_temp(dmi_df, db_engine, uuid_str):
    ...
    with db_engine.connect() as con:
        for row in dmi_df.itertuples():
            ...
            data = ({"farm_id":row.farm_id, "company_name":row.company_name, "pen_type":row.pen_type, "ingredient_name":row.ingredient_name, \
                "as_fed_quantity":row.as_fed_quantity, "as_fed_tons":row.as_fed_tons, "report_date":row.report_date})
...
def write_dmi_to_db(dmi_df, db_engine):
    ...
    upsert_statement = text('INSERT INTO feedwatch_dmi_data(farm_id, company_name, pen_name, report_date, avg_pen_count, '\
        'total_dropped, avg_dropped, total_weighbacks, avg_weighbacks, total_dm, avg_dm_animal) VALUES (:farm_id, '\
        ':company_name, :pen_name, :report_date, :avg_pen_count, :total_dropped, :avg_dropped, :total_weighbacks, '\
        ':avg_weighbacks, :total_dm, :avg_dm_animal) '\
