...
def import_components_from_file(input_df, farm_id):
    ...
    if type(sample_date) != datetime.datetime:
        logging.error("agsource_parser.import_components_from_file(): sample date not registering as datetime.datetime!")
        return False

    # create write_df to hold data from file before writing to database
    db_column_names = ['farm_id', 'sample_date', 'cntl', 'bname', 'sample_id', 'dim', 'milk', 'fat', 'true_prot', 'lactose', 'snf',
    'cells', 'urea', 'mono_unsaturated_fa', 'poly_unsaturated_fa', 'saturated_fa', 'tot_unsaturated_fat', 'scfa',
    'mcfa', 'lcfa', 'trans', 'aceton', 'bhb', 'C14_0', 'C16_0', 'C18_0', 'C18_1', 'de_novo_fa', 'mixed_fa', 'preformed_fa']
...
def append_component_row(farm_id, sample_date, row, write_df):
    ...
    #  'mono_unsaturated_fa', 'poly_unsaturated_fa', 'saturated_fa', 'scfa', 'mcfa', 'lcfa', 'trans', 'fa', 'aceton', 'bhb', 
    ...
    with db_engine.connect() as con:
        ...
        for row in write_df.itertuples():
            ...
            data = ({"farm_id":row.farm_id, "sample_date":row.sample_date, "cntl":row.cntl, "bname":row.bname, \
                "sample_id":row.sample_id, "dim":row.dim, "milk":row.milk, "fat":row.fat, "true_prot":row.true_prot, \
                "lactose":row.lactose, "snf":row.snf, "cells":row.cells, "urea":row.urea, "mono_unsaturated_fa":row.mono_unsaturated_fa, \
                "poly_unsaturated_fa":row.poly_unsaturated_fa, "saturated_fa":row.saturated_fa, \
                "tot_unsaturated_fat":row.tot_unsaturated_fat, "scfa":row.scfa, "mcfa":row.mcfa, "lcfa":row.lcfa, \
...
def import_spectrum_from_file(input_df, farm_id):
    ...
    # create write_df to hold data from file before writing to database
    db_column_names = ['farm_id', 'sample_date', 't_stamp', 'job_type_name', 'sample_id', 'wave_number', 'min_mu', 'max_mu', 'values']
    write_df = pd.DataFrame(columns=db_column_names)
    ...
    for row in input_df.iterrows():
        ...
        if type(row[1].to_list()[2]) == int:
            ...
            if type(row[1].to_list()[0]) == datetime.time:
                ...
                logging.info("agsource_parser.import_spectrum_from_file(): timestamp not registering as datetime or time - inserting sample time to record " + str(row[1].to_list()[2]))
...
def append_spectrum_row(farm_id, timestamp, sample_date, min_mu, max_mu, row, write_df):
    ...
    try:
        if(row[1] == 'Normal'):
            new_row = [int(farm_id), sample_date, timestamp, row[1], int(row[2]), int(row[3]), min_mu, max_mu, grab_spectrum_values_as_array(row, min_mu, max_mu)]
        else:
            new_row = [int(farm_id), sample_date, timestamp, row[1], int(row[2]), None, None, None, [None] * (max_mu - min_mu)]
...
def write_spectrum_to_temp(write_df, db_engine, table_name):
    ...
    with db_engine.connect() as con:
        for row in write_df.itertuples():
            ...
            data = ({"farm_id":row.farm_id, "sample_date":row.sample_date, "t_stamp":row.t_stamp, "job_type_name":row.job_type_name, "sample_id":row.sample_id, \
...
def headers_checkout(input_df):
    ideal_column_names = ['CNTL', 'BNAME', 'SAMP', 'DIM', 'MILK', 'Fat', 'Tru.Prot', 'Lactose', 'SnF', 'Cells', 'Urea', 'MonoUnsaturatedFA', 'PolyUnsaturatedFA', 'SaturatedFA', 'TotalUnsaturatedFA', 'SCFA', 'MCFA', 'LCFA', 'Trans FA', 'Aceton', 'BHB', 'C14_0', 'C16_0', 'C18_0', 'C18_1', 'De novo FA', 'Mixed FA', 'Preformed FA']
...
def get_an_int(input):
    if math.isnan(input):
        return None
    if type(input) == int:
        return input
    elif type(input) in (str, 'unicode'):
        if input == 'nan':
            return None
        else:    
            logging.debug("agsource_parser.get_an_int - converting " + type(input))
            return int(input.replace(',',''))
    else:
        logging.error('agsource_parser.get_an_int - Error converting ' + str(input))
        pdb.set_trace()
        exit(1)
...
def get_a_float(input):
    if type(input) == float:
        ...
        if input == 'nan':
            ...
            logging.debug("agsource_parser.get_a_float - converting " + type(input))
            return float(input.replace(',',''))
    elif type(input) == int:
        return float(input)
    else:
        logging.error('agsource_parser.get_a_float - Error converting ' + str(input))
        exit(1)


def get_a_date(input):
    if type(input) == datetime:
        ...
        logging.error('agsource_parser.get_a_date - Error converting ' + str(input))
        exit(1)
